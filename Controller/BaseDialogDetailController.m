...
@interface BaseDialogDetailController () <JSQMessagesInputToolbarDelegate,
JSQMessagesKeyboardControllerDelegate, EMChatToolbarDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation BaseDialogDetailController

#pragma mark - Class methods

+ (UINib *)nib
{
    return [UINib nibWithNibName:NSStringFromClass([BaseDialogDetailController class])
                          bundle:[NSBundle bundleForClass:[BaseDialogDetailController class]]];
}

+ (instancetype)messagesViewController
{
    return [[[self class] alloc] initWithNibName:NSStringFromClass([BaseDialogDetailController class])
                                          bundle:[NSBundle bundleForClass:[BaseDialogDetailController class]]];
}
...
- (void)viewDidLoad
{
    ...
    self.chatToolbar = [[EaseChatToolbar alloc] initWithFrame:CGRectMake(0, self.collectionView.frame.size.height - chatbarHeight, self.collectionView.frame.size.width, chatbarHeight) type:barType];
    
//    NSLayoutConstraint *constraint = [NSLayoutConstraint constraintWithItem:self.chatToolbar attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:self.collectionView attribute:NSLayoutAttributeWidth multiplier:0 constant:-10];
//    [self.view addConstraint:constraint];
    ...
    self.inputToolbar = self.chatToolbar;
...
- (void)scrollToBottomAnimated:(BOOL)animated
{
    ...
    if (isContentTooSmall) {
        ...
        [self.collectionView scrollRectToVisible:CGRectMake(0.0, collectionViewContentHeight - 1.0f, 1.0f, 1.0f)
                                        animated:animated];
//        NSLog(@"%f,%f, %f,%f,%f", self.collectionView.bounds.origin.x,
//              self.collectionView.bounds.origin.y, self.collectionView.bounds.size.height, self.collectionView.bounds.size.width, collectionViewContentHeight);
        return;
    }
...
- (void)jsq_didReceiveMenuWillShowNotification:(NSNotification *)notification
{
    ...
    NSLog(@"%@", NSStringFromCGRect(selectedCellMessageBubbleFrame));
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (context == kJSQMessagesKeyValueObservingContext) {
        ...
        if (object == self.chatToolbar.inputTextView
...
{
    if (![self.chatToolbar.inputTextView isFirstResponder] && self.toolbarBottomLayoutGuide.constant == 0.0f) {
        return;
    }
...
- (void)jsq_handleInteractivePopGestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
{
    switch (gestureRecognizer.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            self.textViewWasFirstResponderDuringInteractivePop = [self.chatToolbar.inputTextView isFirstResponder];
...
- (void)jsq_adjustInputToolbarForComposerTextViewContentSizeChange:(CGFloat)dy
{
    ...
    if ([self jsq_inputToolbarHasReachedMaximumHeight]) {
        BOOL contentOffsetIsPositive = (self.chatToolbar.inputTextView.contentOffset.y > 0);
...
- (void)jsq_scrollComposerTextViewToBottomAnimated:(BOOL)animated
{
    UITextView *textView = self.chatToolbar.inputTextView;
...
- (void)jsq_updateCollectionViewInsets
{
//    NSLog(@"%f, %f", CGRectGetMaxY(self.collectionView.frame), CGRectGetMinY(self.inputToolbar.frame));
...
- (void)jsq_removeObservers
{
    ...
    @try {
        [self.chatToolbar.inputTextView removeObserver:self
                                                forKeyPath:NSStringFromSelector(@selector(contentSize))
                                                   context:kJSQMessagesKeyValueObservingContext];
    }
